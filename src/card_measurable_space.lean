...
| i := let S := ⋃ j (hij: j < i), generate_measurable_rec j in
...
begin
  unfold generate_measurable_rec,
  apply mem_union_left _ (mem_union_right _ _),
  apply (mem_image compl _ _).mpr,
  simp,
  use j, tauto,
end
...
begin
  ...
  apply mem_range.mp,
  have typf : ∀ n : ℕ, f n ∈ (⋃ j (hij : j < i), generate_measurable_rec s j),
  { intro n, apply mem_Union.2,
    specialize hf n,
    cases hf with j hj,
    use j,
    exact mem_Union.2 hj },
  existsi λ n : ℕ, (⟨f n, typf n⟩:(⋃ j (hij : j < i), generate_measurable_rec s j)),
  tauto
end
...
begin
  ...
  have J : #(⋃ j < i, generate_measurable_rec s j) ≤ (max (#s) 2) ^ aleph_0.{u},
  sorry,
  -- { apply (mk_Union_le _).trans,
  --   have D : (⨆ j < i, #(generate_measurable_rec s j)) ≤ max (# ↥s) 2 ^ aleph_0.{u} := sorry,
  --   apply (mul_le_mul' (trans _ (aleph 1).mk_ord_out.le) D).trans,
  --   rw mul_eq_max A C,
